...
class ApiController extends CController
{
    ...
    public function actioncreatecheckout()
    {
        try {
            ...
            $params = [
                ...
                //'notification_url'=>"https://bastisapp.com/testcode/mercadopago.php?".http_build_query($ipn_params),                
            ...
            try {
                $redirect_url = !empty($absoluteURL)?$absoluteURL: ($is_app?Yii::app()->createAbsoluteUrl("$payment_code/api/cancel"):Yii::app()->createAbsoluteUrl('/account/checkout')) ;            
        ...
        if($app){
            if(!empty($absoluteURL)){
                $this->redirect( $absoluteURL."#checkout?message=".$error );
            } else $this->redirect( Yii::app()->createAbsoluteUrl($payment_code.'/api/failed',['message'=>$error]) );				
    ...
    public function actionprocesspayment()
    {
        try {
            ...
            try {
                $redirect_url = !empty($absoluteURL)?$absoluteURL: ($app? Yii::app()->createAbsoluteUrl("$payment_code/api/cancel") :Yii::app()->createAbsoluteUrl('/account/wallet'));                
                $preference = $client->create($params,$request_options);            
                $payment_reference_id =  $preference->id;       
    ...
    public function actionverifypayment()
    {
        try {
            ...
            $redirect_url = !empty($absoluteURL)?$absoluteURL: ($app? Yii::app()->createAbsoluteUrl("$payment_code/api/cancel") :Yii::app()->createAbsoluteUrl('/account/checkout')) ;           
            ...
            if( $status=="null" || empty($status)){                
                ...
                $redirect_url = !empty($absoluteURL)?$absoluteURL:Yii::app()->createAbsoluteUrl('/');
            ...
            if(!empty($absoluteURL)){
                ...
                $redirect_url = $absoluteURL."/#account/trackorder?".http_build_query($param_url);                    
            } else  {
                if($app){
                    $redirect_url = Yii::app()->createAbsoluteUrl($payment_code.'/api/successful',['message'=>$this->msg]);
                } else {
                    $redirect_url = Yii::app()->createUrl('orders/index',[
    ...
    public function actionverifywallet()
    {
        try {
            ...
            if($model){
                ...
                $this->redirect($successful_url."?message=$message");   
                Yii::app()->end();                
    ...
    public function actionwebhooks()
    {
        try {
            ...
            $http_body = '{"action":"payment.created","api_version":"v1","data":{"id":"1328002605"},"date_created":"2024-11-04T13:58:59Z","id":116871991918,"live_mode":false,"type":"payment","user_id":"131280449"}';
            ...
            switch ($type) {
                case 'payment':                    
                    try {
                        ...
                        $status = $payment->status;
                        // dump("payment->status=>$payment->status");                        
                        // dump("external_reference=>$external_reference");
