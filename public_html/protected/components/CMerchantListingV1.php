			...
			foreach ($res as $val) {
				...
				if(is_array($cuisine_name) && count($cuisine_name)>=1){
					foreach ($cuisine_name as $cuisine_val) {						
						...
						$cuisineName = isset($cuisine[0])?Yii::app()->input->xssClean($cuisine[0]):'';
				...
				if(!empty($val['next_opening'])){
					...
					if(is_array($next_open) && count($next_open)>=1){
						...
						$next_opening = t("Opens [day] at [time]",array(
						 '[day]'=>Date_Formatter::date($next_open_date,"dd MMM",true),
						 '[time]'=>Date_Formatter::Time($next_open_date,"h:mm a",true)
						));
					}
				}
				...
				$val2['cuisines'] = !empty($cuisines)?substr($cuisines,0,-1):'';
