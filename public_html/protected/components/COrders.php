...
class COrders
{
    ...
    public static function orderInfo($lang=KMRS_DEFAULT_LANGUAGE,$datenow='',$is_notifications=false)
    {
    	if(COrders::$order){
    		...
    		$attr = self::orderMeta(array(
			  ...
			  'timezone','custom_field1','custom_field2','payment_name'
    		));				
			...
			if($is_order_ongoing){	
				...
				$estimated_time = CommonUtility::formatDeliveryTimeRange( $currentDate, $order->preparation_time_estimation,$order->delivery_time_estimation,$timezone);
    ...
    public static function getExistingRefund($order_id='')
    {
		...
		$criteria->addInCondition("transaction_type",[
