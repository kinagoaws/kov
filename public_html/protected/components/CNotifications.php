...
class CNotifications
{
	...
	public static function getTotalNotifications($channel='')
	{		
		$chat_count = AR_notifications::model()->count("notication_channel=:notication_channel AND notification_type=:notification_type AND viewed=0
		",[
			':notication_channel'=>$channel,
			':notification_type'=>'chat-message'
		 ]);
		 $alert_count = AR_notifications::model()->count("notication_channel=:notication_channel AND notification_type != :notification_type AND viewed=0",[
			':notication_channel'=>$channel,
			':notification_type'=>'chat-message'
		 ]);
		 return [
			'chat_count'=>intval($chat_count),
			'alert_count'=>intval($alert_count),
		 ];
	}
