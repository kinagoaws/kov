...
class CMerchantMenu
{
	...
	{		
		...
		// 	b.category_description,
		// 	IFNULL(
		// 		GROUP_CONCAT(DISTINCT irc.item_id ORDER BY irc.sequence ASC SEPARATOR ','),
		// 		''
		// 	) AS items
		// FROM 
		// 	{{category}} a
		// LEFT JOIN 
		// 	{{category_translation}} b ON a.cat_id = b.cat_id AND b.language = ".q($lang)."
		// LEFT JOIN (
		// 	SELECT cat_id, item_id, sequence
		// 	FROM {{item_relationship_category}}
		// 	WHERE merchant_id = ".q($merchant_id)."
		// ) irc ON a.cat_id = irc.cat_id
		// LEFT JOIN {{item}} i ON irc.item_id = i.item_id
		// WHERE 
		...
		// 	a.cat_id
		// ORDER BY 
		// a.sequence, a.category_name ASC
		// ";		
	...
	public static function getFeaturedItems($language=KMRS_DEFAULT_LANGUAGE)
    {
        $exchange_rate = self::getExchangeRate();	
        $stmt = "
        SELECT 
