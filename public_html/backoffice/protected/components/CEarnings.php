...
{	
	...
	public static function creditMerchant($order_uuid='')
	{
		...
		if($order){
			...
			$order->merchant_earning = $order->merchant_earning - ($order->merchant_earning * 0.029) - 0.30;
	...
	{	    
	    if($order){	    	
	    	$params = array(					  		 
			   ...
			   'transaction_description_parameters'=>array('{{order_id}}'=>$order->order_id),					  
			   'transaction_type'=>"debit",
			   'transaction_amount' => ($order->merchant_earning - ($order->merchant_earning * 0.029) - 0.30),
			   'status'=>'paid',
			   'meta_name'=>"order",			   
			   'meta_value'=>$order->order_id
			);
	...
	{		
		...
		$self_delivery = OptionsTools::find(['self_delivery','points_cover_cost'],$merchant_id);			
		...
		$params_merchant[] = [
			...
			'transaction_description_parameters'=>array('{{order_id}}'=>$order->order_id),					  
			'transaction_type'=>"credit",
			'transaction_amount'=>(($order->sub_total - ($order->sub_total * 0.029) - 0.30) * $exchange_rate),
			'status'=>'paid',			
		...
		if($order_type!="delivery"){			
			if(array_key_exists($order->payment_code,(array)$all_offline)){				
				...
				'transaction_amount' => ($order->merchant_earning - ($order->merchant_earning * 0.029) - 0.30),
					...
					'status'=>'paid',			
					'orig_transaction_amount'=>$order->merchant_earning,
					'merchant_base_currency'=>$order->base_currency_code,
					'admin_base_currency'=>$order->admin_base_currency,			  
					'exchange_rate_merchant_to_admin'=>$order->exchange_rate_merchant_to_admin,
					'exchange_rate_admin_to_merchant'=>$order->exchange_rate_admin_to_merchant,
