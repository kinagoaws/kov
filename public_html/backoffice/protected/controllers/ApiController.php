...
HTML;
	...
	{	
		...
		try {
			...
			$status_pending = AOrderSettings::getStatus(array('status_new_order','status_cancel_order'));			
			if(in_array($order_status,(array)$status_pending)){				
				try {
			...
			if(in_array($order_status,['deleted','refunded'])){
	...
	public function actionOrderIssueRefund()
	{
		try {
            ...
            if($model_transaction){
                ...
                $model->transaction_name = $refund_type;
                $model->transaction_type = "debit";                
				$model->transaction_description = $refund_type=="full_refund"?"Full refund":"Partial Refund";
                $model->trans_amount = floatval($amount);
                ...
                $model->payment_reference = CommonUtility::generateAplhaCode();
                $model->status = "paid";
				$model->reason = $reason;
                if($model->save()){
					$this->code = 1;
